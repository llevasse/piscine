...
/*   Updated: 2022/08/21 18:08:37 by llevasse         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
t_value	*get_numeric(t_value *dict, t_dictvar *var, int size);
t_value	*get_lexic(t_value *dict, t_dictvar *var, int size);
...
t_value	*get_tab_val(t_value *dict, t_dictvar var, int *file_data)
{
	...
	while (var.i <= file_data[1] + 1)
	{
		...
		while ((var.buff[0] == '\n' || var.buff[0] == ' ') && var.pos > 0)
			var.pos = read(var.fd, var.buff, 1);
		dict = get_numeric(dict, &var, file_data[0]);
		dict = get_lexic(dict, &var, file_data[0]);
	}
	free(var.temp);
	return (dict);
}
